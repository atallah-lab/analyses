...
for (i in names(Hsapiens)[1:24]){
	...
	tmp	<-inrange(insites[ict[which(!is.na(ict[,1])),1],1],exann_i$V2,exann_i$V3) # Check if any Closed-Tight category sites are within the start-end range of exann_i
	tmp2 	<-inrange(insites[ict[which(!is.na(ict[,2])),2],2],exann_i$V2,exann_i$V3) 
	exon_counts[j,1]<-length(which(tmp == TRUE)) + length(which(tmp2==TRUE)) 	  # Fill an element of the exon_counts table with the count
	tmp     <-inrange(insites[icl[which(!is.na(icl[,1])),1],1],exann_i$V2,exann_i$V3) # Check if any Closed-Tight category sites are within the start-end range of exann_i
        tmp2    <-inrange(insites[icl[which(!is.na(icl[,2])),2],2],exann_i$V2,exann_i$V3)
	exon_counts[j,2]<-length(which(tmp == TRUE)) + length(which(tmp2==TRUE))
	tmp     <-inrange(insites[iot[which(!is.na(iot[,1])),1],1],exann_i$V2,exann_i$V3) # Check if any Closed-Tight category sites are within the start-end range of exann_i
	tmp2    <-inrange(insites[iot[which(!is.na(iot[,2])),2],2],exann_i$V2,exann_i$V3)
	exon_counts[j,3]<-length(which(tmp == TRUE)) + length(which(tmp2==TRUE))
        tmp     <-inrange(insites[iol[which(!is.na(iol[,1])),1],1],exann_i$V2,exann_i$V3) # Check if any Closed-Tight category sites are within the start-end range of exann_i
	tmp2    <-inrange(insites[iol[which(!is.na(iol[,2])),2],2],exann_i$V2,exann_i$V3)
